 ...
 * @status      Em processo de homologação
 * @file encode UTF-8
 * @date        2017-12-18
 * 
...
class SantanderRemessa
{
	...
	*/
	public $tamanho_linha = 400;         # se for trabalhar com o layout de 240, entao setar o valor para `false`.
	...
	public $boletos = [];
	public $codigoBanco = '033';         # se for trabalhar com outro banco, deixar vazio essa propriedade.
    public $iRegistros = 0;
	public $aRegistros = [
	    self::HEADER  => [],
	    self::DETALHE => [],
	...
	public $agencia = 9999;               # se a agencia iniciar com zero, torne-a uma string.
    public $conta_movimento = 99999999;   # Conta movimento Beneficiário 8 posicoes
    public $conta = 99999999;             # Conta cobrança Beneficiário 8 posicoes
	public $contaDv;
    public $carteira = 101;
     ...
     * @return array|bool
     *
     * Precisamos do arquivo de remessa para poder usar esse metodo.
     */
    /*
    ...
    */
    ...
    public function getNossoNumero()
    {
        if (empty($this->campoNossoNumero)) {
            return $this->campoNossoNumero += 1; //$this->gerarNossoNumero();
        }
     ...
     * Santander
     */
    public function santanderContaCorrente($agencia, $contaCorrente)
    {
        $n = $this->numberFormatGeneral($agencia, 4, 0) . '00' . $this->numberFormatGeneral($contaCorrente, 8, 0);
        $chars = array_reverse(str_split($n, 1));
     ...
     * > Quantidade de documentos no arquivo $add=0 (não contamos o header e o trailer, apenas o detalhe)
     *
     * @return int
     */
    ...
    /**
     * --
     * precisa ter o tamanho de 400 posicoes para o layout CNAB400, o valor esta setado no
     * inicio da classe pois vamos ter apenas o padrao CNAB400
     * --
	...
	/***********************************************************************************************
	 * @CNAB400 REMESSA methods Santander Bank
    ...
    public function getCodigoTransmissao()
    {
        ...
        // . $this->formatCnab('9', substr($this->getCodigoCliente(), 0, 7), 8)  estava no lugar de $conta_movimento
        return $this->formatCnab('9', $this->getAgencia(), 4)
             . $this->formatCnab('9', $this->conta_movimento, 8)
             . $this->formatCnab('9', $conta, 8);
    }
    ...
    public function header()
    {
        ...
        $this->add(27, 46, $this->formatCnab('9', $this->getCodigoTransmissao(), 20));
        //$this->add(47, 76, $this->formatCnab('X', $this->getBeneficiario()->getNome(), 30));
    ...
    public function addBoleto($boleto)
    {
        $this->boletos[] = $boleto;
        $this->iniciaDetalhe();
        $this->total += $boleto['valor'];
        $this->add(1, 1, '1');
        //$this->add(2, 3, strlen($this->numbersOnly($this->getDocumento())) == 14 ? '02' : '01');
        $this->add(2, 3, '02');
        $this->add(4, 17, $this->formatCnab('9L', $this->getDocumento(), 14));
        $this->add(18, 37, $this->formatCnab('9', $this->getCodigoTransmissao(), 20)); // inclui Código da agência, Conta Movimento e COnta cobranca do Beneficiário.
        $this->add(38, 62, $this->formatCnab('X', $this->getNumeroControle(), 25)); // numero de controle da remessa
        $this->add(63, 70, substr($this->numbersOnly($boleto['numero']), -8));
        $this->add(71, 76, '000000');
        $this->add(77, 77, '');
        $this->add(78, 78, ($boleto['multa'] > 0 ? '4' : '0'));
        $this->add(79, 82, $this->formatCnab('9', $boleto['multa'], 4, 2));
        $this->add(83, 84, '00');
        $this->add(85, 97, $this->formatCnab('9', 0, 13, 2));
        $this->add(98, 101, '');
        //$this->add(102, 107, $boleto['juros'] === false ? '000000' : $boleto->getDataVencimento()->copy()->addDays($boleto['juros'])->format('dmy'));
        $this->add(102, 107, $boleto['juros'] === false ? '000000' : date('dmy', strtotime('+6 days')));
        ...
        //$this->add(111, 120, $this->formatCnab('X', $this->getNumeroDocumento(), 10));
        $this->add(111, 120, $this->formatCnab('X', $boleto['numeroDocumento'], 10));
        $this->add(121, 126, $boleto['dataVencimento']);
        $this->add(127, 139, $this->formatCnab('9', $boleto['valor'], 13, 2));
        ...
        $this->add(161, 173, $this->formatCnab('9', $this->percent($boleto['valor'], $percent=2), 13, 2)); // Valor de mora a ser cobrado por dia de atraso
        ...
        $this->add(352, 381, $this->formatCnab('X', $boleto['sacadorAvalista'] ? $boleto['sacadorAvalista']['nome'] : '', 30));
        $this->add(382, 382, '');
        $this->add(383, 383, 'I');
        $this->add(384, 385, substr($this->getConta(), -1) . $this->santanderContaCorrente($this->getAgencia(), $this->getConta()));
...
//$linha_remessa = $banco->save($path='/var/www/test/remessa/remessa.txt');
//var_dump($linha_remessa);
 ...
 */
// sua consulta com os dados dos boletos gerados no dia
 ...
 */
//$boleto = []; # esvaziar boleto antes de iniciar
foreach ($data_boleto as $key => $value) 
{
    $boleto = array(
        //'logo' => realpath(__DIR__ . '/../logos/') . DIRECTORY_SEPARATOR . '033.png',
        'dataVencimento' => date('dmy', strtotime('+3 days')),
        'valor' => $value['valor_cobrado'],               # 100.23
        'multa' => false,
        'juros' => false,
        'moraDia' => false,                               # cobrar 2% do valor do boleto apos o vencimento (tratado acima).
        'desconto' => 0,                                  # se valor do desconto maior que 0 entao precisa ter uma data para a concessao do desconto.
        'data_desconto' => false,                         # usar apenas se o desconto for maior que zero.
...
echo $linha_remessa = $banco->save($path='remessa/' . $fn);
?>
